...
def create_data():
    create_data_rows()
    load_data()
    return JSONResponse(content={"message": "Data loaded successfully"})

@app.post("/create_view")
def create_view():
    create_view_table()
    return JSONResponse(content={"message": "View created successfully"})
...
app.get("/")
def read_root():
    return JSONResponse(content={"message": "Welcome to the Employee Summary API"})